...
import { LayoutDashboard, Users, UserPlus, FileCheck, ListTodo, FileText, LogOut, Menu, X, Building2, Settings, FileArchive, AlertTriangle, Clock } from 'lucide-react';
...
const navItems = [
  ...
  { path: '/notices', label: 'Notices', icon: AlertTriangle },
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-16'} bg-white border-r border-gray-200 flex flex-col transition-all duration-200`}>
        ...
        <nav className="flex-1 py-2 overflow-y-auto">
          {navItems.map(({ path, label, icon: Icon }) => (
            <Link key={path} to={path} className={`flex items-center gap-3 px-4 py-2 mx-2 rounded-lg transition-colors ${location.pathname === path ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
              <Icon className="w-5 h-5 flex-shrink-0" />{sidebarOpen && <span className="text-sm">{label}</span>}
            </Link>
          ))}
        </nav>
        <div className="p-4 border-t">
          {sidebarOpen && <div className="mb-2 text-sm"><p className="font-medium text-gray-800">{user.full_name}</p><p className="text-xs text-gray-500 capitalize">{user.role}</p></div>}
